...
def test_patch_nonexistent_movie(api_manager):
    response = api_manager.movies_api.update_movie(99999999, {"name": "Updated"})
    assert response.status_code == 404


def test_patch_invalid_id(api_manager):
    response = api_manager.movies_api.update_movie("abc", {"name": "Updated"})
...
def test_patch_invalid_data(api_manager, movie_payload):
    ...
    response = api_manager.movies_api.update_movie(movie_id, {"price": -100})
